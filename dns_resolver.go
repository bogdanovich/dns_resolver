...
// based on miekg/dns
// forked from bogdanovich/dns_resolver to add IPv6 support
...
func (r *DnsResolver) LookupHost(host string) ([]net.IP, error) {
	return r.lookupHost(host, r.RetryTimes, false)
}

func (r *DnsResolver) LookupHost6(host string) ([]net.IP, error) {
	return r.lookupHost(host, r.RetryTimes, true)
}

func (r *DnsResolver) lookupHost(host string, triesLeft int, ipv6 bool) ([]net.IP, error) {
	m1 := new(dns.Msg)
	m1.Id = dns.Id()
	...
	if err != nil {
		if strings.HasSuffix(err.Error(), "i/o timeout") && triesLeft > 0 {
			triesLeft--
			return r.lookupHost(host, triesLeft, ipv6)
		}
		return result, err
	}
